...
<div style="margin-left: 80px;" id="main-content" class="container mt-4">
    ...
    <a href="{{ route('admin.research-category.create') }}" class="btn">Add New Category</a>
    ...
    <ul class="list">
        @foreach($categories as $category)
            <li style="display: flex; justify-content: space-between; align-items: center;">
                {{ $category->category }}
                <form action="{{ route('admin.research-category.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this category?');">
                    @csrf
                    @method('DELETE')
